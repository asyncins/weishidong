...
打开一个自己的 GitHub 仓库（以我为例，也就是我存放专栏应用的仓库），选择选项卡中的 `Actions` 项：
...
这里要注意的是 `SSH_PRIVATE_KEY`，它的值 `${{ secrets.VUEPRESS_SERVER_PRI_KEY }}` 其实是保存在 GitHub Secret 中的服务器密钥，这个密钥就是用来实现免密登录效果的。
...
至此，GitHub Actions 部署 Node 应用到云服务器的实践就完成了。
<Vssue :title="$title" />
